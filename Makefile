...
gen-client: ## gen client code of {svc}. example: make gen-client svc=product
	@cd rpc_gen && cwgo client --type RPC --service ${svc} --module github.com/MoScenix/ai-code/rpc_gen  -I ../idl  --idl ../idl/${svc}.proto

.PHONY: gen-server
gen-server: ## gen service code of {svc}. example: make gen-server svc=product
	@cd app/${svc} && cwgo server --type RPC --service ${svc} --module github.com/MoScenix/ai-code/app/${svc} --pass "-use github.com/MoScenix/ai-code/rpc_gen/kitex_gen"  -I ../../idl  --idl ../../idl/${svc}.proto
